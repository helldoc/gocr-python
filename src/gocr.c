...
static void out_version(int v) {
  fprintf(stderr, " Optical Character Recognition --- gocr "
          ...
          " Copyright (C) 2001-2013 Joerg Schulenburg  GPG=1024D/53BDFBE3\n"
...
static char **get_recognized_characters(job_t *job) {
  const char *line;
  ...
  for(int i = 0; i < linecount; ++i) {
    ...
    string_array[i] = malloc(linelen + 1); // len + 1 byte Including \0 characters
    strcpy(string_array[i], line);
  }
  string_array[linecount] = NULL; // termination pointer
  free_textlines(&(job->res.linelist));
  return string_array;
}
...
char **process_path(const char *path) {
  ...
  while (multipnm==1) { /* multi-image loop */
    ...
    result = get_recognized_characters(job);

    job_free_image(job);

  }

  if(multipnm) {/* -1=255 on error, 0 ok */
    fputs("Bad processing in process_path", stdout);
    result = NULL;
  }
  return result;
}
